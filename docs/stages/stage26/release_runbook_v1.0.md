...
5. **Execute migration** — use the automated migration runner; no manual production SQL. The historical Stage23 `007 ... v1.0` file is forbidden; only v1.1 is canonical, followed by the additive Stage27 migration 008. By default the runner follows schema migration with the canonical Stage12 reference seed and the audited canonical Question Bank publication; verify 1,806 `PUBLISHED` and serving rows. `--schema-only` is an explicit diagnostic opt-out, not the normal deployment path.
