...
function mergeSummaries(
  ...
  incoming: ReviewSummary[],
): ReviewSummary[] {
  const map = new Map(current.map((summary) => [summary.id, summary]));
  for (const summary of incoming) map.set(summary.id, {...map.get(summary.id), ...summary});
  return [...map.values()];
}

function priorityFor(summary: ReviewSummary | null): Priority {
  ...
  const overdueHours = Number.isFinite(dueMs) ? (Date.now() - dueMs) / 3_600_000 : 0;
  if (repeats >= 3 || overdueHours >= 24) return "high";
  if (repeats >= 1 || overdueHours > 0) return "medium";
  return "low";
}
...
function optionClass(
  ...
  if (feedback) {
    ...
    else if (optionId === feedback.selected_option_id && !feedback.is_correct) parts.push(styles.optionIncorrect);
    else parts.push(styles.optionMuted);
  }
  return parts.join(" ");
}
...
function priorityLabel(priority: Priority, isFa: boolean) {
...
export function ReviewRunnerClient({
  ...
  const ensureSession = useCallback((queue: ReviewSummary[], loadedItem: ReviewItem) => {
    ...
    if (saved) {
      ...
      const next: ReviewSession = {
        ...
        summaries: mergeSummaries(saved.summaries, [...dueQueue, fallback]),
      };
      persistSession(next);
      return;
    }
    ...
    const created: ReviewSession = {
      ...
      order,
      summaries: mergeSummaries(dueQueue, [fallback]),
      answers: [],
  ...
  const priority = priorityFor(currentSummary);
  ...
  const navigateTo = useCallback((targetId: string, targetCursor: number) => {
    if (session) {
      const next = {...session, cursor: targetCursor};
      persistSession(next);
    }
  ...
  const toggleRepeatInSession = useCallback(() => {
    ...
    if (alreadyRequested) {
      ...
      const canRemoveTail = lastIndex > currentIndex && !session.answers.some((answer) => answer.cursor === lastIndex);
      const next: ReviewSession = {
        ...session,
        order: canRemoveTail
  ...
  const scheduleMessage = useMemo(() => {
    ...
    const {feedback: gradedFeedback} = feedbackData;
    const schedule = scheduleAfter;
    if (!schedule) {
      return gradedFeedback.is_correct
  ...
  const misconceptionLabel = currentSummary?.group_key
    || (selectedText && correctText && selectedText !== correctText ? `${selectedText} ↔ ${correctText}` : null)
    || currentSummary?.title;
  const ruleText = feedback?.correct_option_explanation || feedback?.full_explanation || null;
  const impactDelta = scheduleBefore && scheduleAfter
    ? scheduleAfter.interval_days - scheduleBefore.interval_days
    : null;
      ...
      <aside dir={isFa ? "rtl" : "ltr"} className={styles.inboxPanel} aria-label={isFa ? "فهرست جلسه مرور" : "Review session list"}>
        ...
        <div className={styles.queueList}>
          {queueRows.map(({id, index, summary, answered}) => {
            const rowPriority = priorityFor(summary ?? null);
            ...
            return (
              ...
              >
                <span className={`${styles.priorityBadge} ${styles[`priority_${rowPriority}`]}`}>
                  {priorityLabel(rowPriority, isFa)}
                </span>
                ...
                <span className={`${styles.repeatBubble} ${answered ? (answered.is_correct ? styles.repeatBubbleCorrect : styles.repeatBubbleIncorrect) : ""}`}>
                  {formatNumber(summary?.repeat_count ?? 0, isFa)}/{formatNumber(Math.max(1, (summary?.repeat_count ?? 0) + 1), isFa)}
                </span>
              </button>
            );
          })}
        ...
        {item.reviewability === "HISTORY_ONLY" && !feedback ? (
          <section className={styles.historyOnlyCard}>
            ...
            <div>
              ...
              <p>{isFa ? "سؤال دیگر برای تلاش مجدد ایمن نیست و در آزمون جدید استفاده نمی‌شود." : "This question is no longer safe for another retry and is excluded from new tests."}</p>
          ...
          <section className={styles.questionCard} aria-labelledby="review-question">
            <div className={styles.questionTopline}>
              <div className={styles.tagRow}>
                ...
                <span className={`${styles.priorityBadge} ${styles[`priority_${priority}`]}`}>
                  {isFa ? "اولویت" : "Priority"} {priorityLabel(priority, isFa)}
                </span>
              </div>
              {previousAnswer ? (
            ...
            {feedback ? (
              <div className={`${styles.feedbackCard} ${feedback.is_correct ? styles.feedbackCorrect : styles.feedbackIncorrect}`} aria-live="polite">
                <div className={styles.feedbackHeading}>
                  ...
                  <div>
                    <strong>{feedback.is_correct ? (isFa ? "درست پاسخ دادید" : "Correct") : (isFa ? "نیاز به مرور دوباره" : "Needs another review")}</strong>
            ...
            <div className={styles.questionFooter}>
              <button
                ...
                aria-pressed={session?.repeat_requested_ids.includes(reviewId) ?? false}
                onClick={toggleRepeatInSession}
              >
                <span aria-hidden="true">↻</span>
                {session?.repeat_requested_ids.includes(reviewId)
                  ? (isFa ? "تکرار این جلسه لغو شود" : "Remove session repeat")
      ...
      <aside dir={isFa ? "rtl" : "ltr"} className={styles.insightColumn} aria-label={isFa ? "تحلیل جلسه مرور" : "Review session insights"}>
        <section className={styles.progressCard} style={progressStyle}>
          ...
          <div className={styles.progressBody}>
            ...
            <dl className={styles.statsList}>
              <div><dt>{isFa ? "سؤالات پاسخ داده‌شده" : "Answered"}</dt><dd>{formatNumber(answeredCount, isFa)}/{formatNumber(totalItems, isFa)}</dd></div>
        ...
        <section className={styles.learningCard}>
          <div className={styles.learningHeader}>
            <h2>{isFa ? "تمرکز یادگیری" : "Learning focus"}</h2>
            <span className={`${styles.priorityBadge} ${styles[`priority_${priority}`]}`}>
              {priorityLabel(priority, isFa)}
            </span>
          </div>
          {feedback ? (
            <>
              <div className={styles.insightBlock}>
                <span className={styles.insightLabel}>{isFa ? "اشتباه محتمل شما" : "Likely misconception"}</span>
                <strong className={styles.misconception} dir="ltr">{misconceptionLabel}</strong>
                <small>{isFa ? `${formatNumber(currentSummary?.repeat_count ?? 0, true)} بار مشاهده شده` : `Seen ${currentSummary?.repeat_count ?? 0} times`}</small>
                {feedback.selected_option_explanation ? <p>{feedback.selected_option_explanation}</p> : null}
              </div>
              <div className={styles.insightBlock}>
                <span className={styles.insightLabel}>{isFa ? "قاعده مرتبط" : "Related rule"}</span>
                <p dir="auto">{ruleText ?? (isFa ? "توضیح قاعده همراه پاسخ ثبت نشده است." : "No related-rule explanation was returned with this answer.")}</p>
              </div>
              <div className={styles.insightBlock}>
                <span className={styles.insightLabel}>{isFa ? "اثر بر تسلط" : "Mastery impact"}</span>
                <div className={styles.impactStatus}>
                  <span aria-hidden="true">✓</span>
                  <strong>{isFa ? "شاهد مرور جدید ثبت شد" : "New review evidence recorded"}</strong>
                </div>
                {scheduleAfter ? (
                  <div className={styles.impactRow}>
                    <strong>{formatNumber(scheduleAfter.consecutive_correct_reviews, isFa)}</strong>
                    <span>{isFa ? "مرور صحیح متوالی" : "consecutive correct reviews"}</span>
                  </div>
                ) : null}
                {impactDelta !== null ? (
                  <small>
                    {isFa
                      ? `تغییر فاصله مرور: ${impactDelta >= 0 ? "+" : ""}${formatNumber(impactDelta, true)} روز`
                      : `Interval change: ${impactDelta >= 0 ? "+" : ""}${impactDelta} days`}
                  </small>
                ) : null}
                <small>{isFa ? "امتیاز آزمون اصلی بازنویسی نمی‌شود." : "The original test score is not rewritten."}</small>
              </div>
            </>
          ) : (
            <div className={styles.preFeedbackHint}>
              <span aria-hidden="true">◎</span>
              <p>{isFa ? "ابتدا از حافظه پاسخ دهید؛ سپس misconception، قاعده مرتبط و اثر مرور نمایش داده می‌شود." : "Answer from memory first; misconception, related rule and learning impact appear after grading."}</p>
            </div>
          )}
        </section>
