-- Grammar Mastery Platform - Stage 23 auditable import pipeline storage
-- Migration version: stage23-import-storage-v1.0.0 | PostgreSQL 15+
-- Additive. Stage 12 questions/audit_logs and Stage 20 admin_audit_events remain canonical.
...
CREATE INDEX IF NOT EXISTS idx_s23_import_batches_status_time
  ON import_batches(status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_s23_import_batches_raw_hash
  ON import_batches(raw_sha256, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_s23_import_rows_disposition
  ON import_batch_rows(batch_id, disposition, row_number);
CREATE INDEX IF NOT EXISTS idx_s23_import_rows_fingerprint
  ON import_batch_rows(fingerprint_sha256) WHERE fingerprint_sha256 IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_s23_import_rows_semantic
  ON import_batch_rows(semantic_signature_sha256) WHERE semantic_signature_sha256 IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_s23_import_events_batch_time
  ON import_batch_events(batch_id, occurred_at, id);

CREATE OR REPLACE FUNCTION s23_prevent_import_event_mutation() RETURNS trigger AS $$
BEGIN
  RAISE EXCEPTION 'Stage23 import events are append-only';
END; $$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_s23_import_events_append_only ON import_batch_events;
CREATE TRIGGER trg_s23_import_events_append_only
BEFORE UPDATE OR DELETE ON import_batch_events
FOR EACH ROW EXECUTE FUNCTION s23_prevent_import_event_mutation();

CREATE OR REPLACE FUNCTION s23_guard_import_batch_transition() RETURNS trigger AS $$
BEGIN
  IF NEW.id<>OLD.id
    ...
    RAISE EXCEPTION 'Stage23 import batch identity and raw provenance are immutable';
  END IF;
  IF NOT (
    ...
    RAISE EXCEPTION 'Invalid Stage23 import batch state transition % -> %', OLD.status, NEW.status;
  END IF;
  RETURN NEW;
END; $$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_s23_import_batch_transition ON import_batches;
CREATE TRIGGER trg_s23_import_batch_transition
BEFORE UPDATE ON import_batches
FOR EACH ROW EXECUTE FUNCTION s23_guard_import_batch_transition();
...
VALUES
  ...
  ('stage23.import_storage','stage23-import-storage-v1.0.0','REVIEW_CANDIDATE','database/postgres/007_stage23_import_pipeline_v1.0.sql','{"raw_provenance":"OBJECT_KEY_PLUS_SHA256","events":"APPEND_ONLY"}'::jsonb)
